...
class RolesTableSeeder extends Seeder
{
    ...
    public function run()
    {
        $this->commonConfig = $this->fs->getRequire(
            app('path.common') . '/resources/defaults/permissions.php',
        );
        $this->appConfig = $this->fs->getRequire(
    ...
    {
        $defaultPermissions = collect($appRole['permissions'])->map(function (
            $permission,
        ) {
        ...
        $dbPermissions = $this->permission
            ->whereIn('name', $defaultPermissions->pluck('name'))
            ->get();
    ...
    private function attachUsersRoleToExistingUsers()
    {
        ...
        $this->user
            ...
            ->select('id')
            ->chunk(500, function (Collection $users) use ($role) {
                $insert = $users
